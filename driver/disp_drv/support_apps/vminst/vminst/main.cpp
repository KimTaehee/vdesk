...
int __cdecl _tmain(int argc, _TCHAR *argv[])
{
	...
	if (!strcmp(argv[1], "-i")) {
		...
		//Vista and Win7이면 미러모드를 켜고, 이외라면 미러모드를 끈다
		FixInfFile(INF);
		...
		// registery를 탐색하여 해당 드라이버의 미러모드 오프
		if (isVista || isWin7) {
			DisableMirror(3);
		}
