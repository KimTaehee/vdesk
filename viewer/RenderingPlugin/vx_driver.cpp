...
struct vx_state{
	struct vx_mon_state monitor[30];
	int max_mon;
};
...
void i_vx_add_device(vx_state *vx,DISPLAY_DEVICE dd)
{
	...
	else
	{
		...
		vx->monitor[vx->max_mon].res.x = *(((int*)vx->monitor[vx->max_mon].DeviceVideoMemory));
		vx->monitor[vx->max_mon].res.y = *(((int*)vx->monitor[vx->max_mon].DeviceVideoMemory)+1);
...
int vx_mon_tokenGet(vx_mon_state *mon)
{
	int res;
